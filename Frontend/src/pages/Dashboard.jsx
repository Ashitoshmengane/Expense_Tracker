...
import AddTransactionModal from '../components/Add'
    ...
    // to convert daet to ISO timelinex
...
const Dashboard = () => {
 ...
 const { 
  ...
  const financialOverviewData = useMemo(() => {
    ...
    if (
      timeFrame === "monthly" &&
      overviewMeta.spendByCategory &&
      Object.keys(overviewMeta.spendByCategory).length > 0
    ) {
      return Object.entries(overviewMeta.spendByCategory).map(([category, amount]) => ({
        name: category,
    ...
    const expenseSource =
      timeFrame === "monthly" && Array.isArray(overviewMeta.recentTransactions)
        ? overviewMeta.recentTransactions.filter((t) => t.type === "expense")
        : filteredTransactions;

    const categories = {};
    expenseSource.forEach((transaction) => {
      if (transaction.type === "expense") {
        categories[transaction.category] =
          (categories[transaction.category] || 0) + Number(transaction.amount || 0);
      }
    });
  ...
  // build server-provided reent list 
        ...
        <div className={dashboardStyles.summaryGrid}>
            ...
            additionalContent={
                <div className='flex items-center gap-2 mt-2 text-sm'>
                    ...
                    {/* <span className={dashboardStyles.expenseBadge}>-${map.round(displayExpenses.toLocaleString())} */}
                    <span className={dashboardStyles.expenseBadge}>-${Math.round(displayExpenses).toLocaleString()}
